...
- **🗑️ REMOVED EXPERIMENTAL FEATURES**: Deleted Advanced Analytics system (2000+ lines) that was added via git merge. Removed ML dependencies (scikit-learn, numpy, pandas, redis - 150MB+) and associated code. Feature had 22.7% failure rate, duplicated existing capabilities (Coinglass indicators, Smart Money, MSS), and added unnecessary complexity. System now cleaner and more focused on proven, production-ready features.
- **📚 EXTENDED USER GUIDE**: Expanded GPT_QUESTION_TEMPLATES.txt from 243 to 1,500+ lines with comprehensive user guide including feature deep dives, real-world workflows, interpretation guides, decision trees, common mistakes, and advanced strategies. Complete tutorial for all 8 core features in single file.
