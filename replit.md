...
- **Enhanced Logging System (2025-11-17)**: Comprehensive logging improvements for production monitoring and debugging. Timezone converted from UTC to WIB (UTC+7) for Indonesia-based operations. Implemented `DetailedRequestLoggerMiddleware` that captures all HTTP request details: method, endpoint, query parameters, request body (with proper body restoration for POST/PUT/PATCH), response status, duration, client IP, and user-agent. Includes security-focused sensitive data redaction that automatically masks api_key, password, token, secret, authorization, and other credentials in logs using recursive `redact_sensitive_data()` function. All logs now emit structured JSON with WIB timestamps for consistent timezone tracking. Critical fix applied to prevent request body consumption - middleware uses custom receive() callable to re-inject body bytes, ensuring downstream handlers can still read request payload. Middleware registered after CORS to maintain proper request flow. Optimized for ChatGPT/GPT Actions integration debugging with detailed request tracking including request_id for correlation.
- **4-Phase Automated Scanning & Performance Tracking System (2025-11-17)**: Comprehensive 24/7 market monitoring infrastructure deployed via Pull Request #2 (8,813 insertions, 23 files). **Phase 1 - Auto-Scanner with APScheduler**: Background task scheduler running 3 automated scans (Smart Money hourly, MSS 6-hourly, RSI 4-hourly) with configurable intervals via environment variables. Implements job scheduling, error recovery, and automatic signal history storage. **Phase 2 - Parallel Batch Processor & Smart Cache**: High-performance scanner achieving 10x improvement (1000 coins in <6 minutes). Features 3-layer caching system (L1 LRU in-memory, L2 Redis, L3 Database) with intelligent TTL management for 5 data types. Includes concurrent batch processing with rate limiting (10 req/s) and max concurrency (50 workers). New endpoints: `/batch/parallel-scan`, `/batch/parallel-scanner/stats`, `/batch/cache/stats`, `/batch/cache/warm`. **Phase 3 - Unified Ranking System**: Composite 0-100 scoring combining 6 signal sources (Smart Money, MSS, RSI, Social, Whale, Volume) with weighted contributions (Smart Money 30%, MSS 25%, RSI 15%, Social 15%, Whale 10%, Volume 5%). Features cross-validation across 4 scanners, 6-tier classification (TIER_1_EXTREME_LONG to TIER_6_EXTREME_SHORT), and multi-timeframe analysis. New endpoints: `/unified/score/{symbol}`, `/unified/top-opportunities`, `/unified/scan-market`. **Phase 4 - Performance Validation & Analytics**: Automated outcome tracking at 5 intervals (1h, 4h, 24h, 7d, 30d) with WIN/LOSS/NEUTRAL classification based on P&L thresholds. Includes win rate calculation, average P&L metrics, tier performance comparison, and alert validation. New endpoints: `/performance/stats`, `/performance/analyze`, `/performance/tier-performance`, `/performance/scanner-performance`. System now provides complete market scanning automation with performance validation, deployed and actively running in production with APScheduler dependency installed.
